...
( docker ps -q --filter "name=noetic" | grep -q . &&  docker stop noetic || true) && \
 ...
 docker run -d -p11311:11311 -p9090:9090 --rm --net=ros --env="DISPLAY=novnc:0.0" --name noetic maiquelb/embedded-mas-ros:0.8a \
/bin/bash -c "
set -e
...
" 
cd random_turtlebot3 &&  ./memory_monitor_jason.sh
